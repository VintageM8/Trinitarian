...
using Trinitarian.Items.Weapons.Ranged.Bows;
using Trinitarian.Items.Weapons.Ranged.Guns;
...
namespace TerrariaDemised.Drops
{
    ...
    { 
        public override void NPCLoot(NPC npc)
        {
            ...
            if (npc.type == NPCID.IceBat || npc.type == NPCID.IceSlime)          
            {
             ...
             if (npc.type == NPCID.AngryNimbus)          
            {
                 ...
                 if (Main.rand.NextFloat() < .15f) 
                 {
                    Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ModContent.ItemType<StormEnergy>(), 1);
                 }
            }
